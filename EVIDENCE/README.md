...
| `../lean/mutate/mutation-lean.log` | eight deliberate defects injected into the Lean definitions; the **runnable suite** is `lean/mutate/mutate-lean.sh` | **8 applied, 8 killed, 0 survived, 0 discarded**, re-run for this release with **per-theorem attribution** — the log names which theorems died for each mutation. The harness has its own negative control (`GF_MUTATE_SELFTEST=1`) which must report a SURVIVOR and exit 1; measured, it does. A theorem no mutation kills is decorative; these are not. |
...
| `../lean/Proofs/` | the four Lean 4 sources | Published so the transcript above can be re-run rather than believed. They import Mathlib; see `lean/README.md` for what each proves and what the mutations killed. **CI does not verify them and does not claim to.** |
