...
> namespaced form — `/rot-dtd-goal:goal-status` — while interactive sessions
...
That is the engine's own output replayed against snapshots of a real bench:
three queued goals (build → extend → harden a pure-bash CSV tool, 22 sealed
criteria) worked end-to-end by real Claude Code sessions with this plugin
installed. Only the pacing is staged. The full record — the untouched journal,
the ledger, the queue and the measured numbers — is in
[`EVIDENCE/bench-goal/`](EVIDENCE/bench-goal/summary.log).

| what the bench measured | result |
|---|---|
| goals completed | 3 of 3, all through the gate's CONFIRM sweep |
| queue hand-off | happened **live in-session** — the gate blocked with the next goal instead of allowing the stop |
| gate refusals | a full block at iteration 1 (10 of 10 failing), targeted feedback only |
| tamper guard | **4** denied attempts to write `.claude/goal/*`, journalled |
...
Eighteen laws are **declared** in `hooks/trust_contract.dtd`, and
